#R Studio data visualization and analysis project
#Malika Gupta
library(shiny)
...
#setwd("/C:/Users/malik/Desktop/BigData2/Big Data 2 - Project 1/ITE5424.R")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Machine Learning Classification", tabName = "model_evaluation", icon = icon("line-chart"))
    )
  ),
  dashboardBody(
    fluidRow(
      column(width = 12,
             ...
             h3("Presented by Malika Gupta (N01581424) ", align = "center"),
             br()
      )
    ),
    tabItems(
      tabItem(tabName = "overview",
              fluidRow(
                box(
                  title = "Exploratory Analysis", status = "primary", solidHeader = TRUE,
      ...
      tabItem(tabName = "flight_distance",
              fluidRow(
                box(
                  title = "Flight Distance by Class", status = "primary", solidHeader = TRUE,
...
server <- function(input, output) {
  ...
  output$summary <- renderPrint({
    ...
    # Print the summary_text to debug
    print(summary_data)
  ...
  output$onlineBoardingBarChart <- renderPlot({
    ggplot(Airline_customer_satisfaction, aes(x = Online.boarding, fill = satisfaction)) +
      ...
      labs(title = "Satisfaction by Online Boarding Experience",
           x = "Online Boarding Experience (A scale of 0 to 5)", y = "Count") +
